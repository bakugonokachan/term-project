...
由於加入旋轉後，速度分量之間會產生耦合，無法維持和前兩種模型同樣簡潔的解析解形式，因此本程式使用 **RK4（四階 Runge–Kutta）數值積分**來模擬「線性阻力 + 旋轉」情境，並以 \(y<0\) 作為落地停止條件，確保模擬條件正確。
最後執行程式時，將rpm調至0時確實與只考慮空氣阻力之情形相同（軌跡重合），且在高轉速(3000rpm)時觀察到軌跡在落地前軌跡急墜，切實如同現實中棒球的splitter。
